...
/*   Updated: 2024/10/16 08:12:34 by dosokin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	render_enemy(char **map, t_game_data *game_data)
{
	...
	if (mlx_image_to_window(mlx_data->mlx, mlx_data->images.enemy_img1,
			enemy_coord.x * 64 + 20, enemy_coord.y * 64 + 20) < 0)
		return (error_manager(MLX));
	if (mlx_image_to_window(mlx_data->mlx, mlx_data->images.enemy_img2,
			enemy_coord.x * 64 + 20, enemy_coord.y * 64 + 20) < 0)
		return (error_manager(MLX));

...
int render_counter(struct s_game_data *game_data)
{
    ...
    while (i < 4)
    {
        ...
        while (j < 10)
        {
            image = *(get_num_location(j, num_images));
...
static int disable_counter(struct s_game_data *game_data)
{
    ...
    while (i < 4)
    {
        ...
        while (j < 10)
        {
            (*(get_num_location(j, num_images)))->enabled = false;
            j++;
        }
        i++;
    }
...
void update_counter(struct s_game_data *game_data, size_t move_count)
{
    ...
    while (i < 4)
    {
        ...
        if (i < len_n)
        {
            image = *(get_num_location(move_count % 10, num_images));
            image->enabled = true;
            move_count /= 10;
        }
        else
            (*get_num_location(0, num_images))->enabled = true;
        i++;
    }
}
